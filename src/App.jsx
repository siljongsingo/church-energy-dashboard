...
function MTable({ year, data, c1, c2, lbs, mob }) {
  const lm = Math.max(
    ...c1.fs.map((f) => lastM(data, year, f)),
    ...c2.fs.map((f) => lastM(data, year, f))
  );
        ...
        <table style={{ width: "100%", fontSize: 11, borderCollapse: "collapse", minWidth: 460 }}>
          <thead>
            ...
            <tr style={{ borderBottom: "0.5px solid var(--color-border-secondary)" }}>
              {c1.sl.map((l, i) => (
                <th key={i} style={{ padding: "4px 7px", textAlign: "right", color: c1.tc, fontWeight: 400, fontSize: 10, background: c1.bg, borderLeft: i === 0 ? "2px solid " + c1.bd : "none" }}>{l}</th>
              ))}
              {c2.sl.map((l, i) => (
                <th key={i} style={{ padding: "4px 7px", textAlign: "right", color: c2.tc, fontWeight: 400, fontSize: 10, background: c2.bg, borderLeft: i === 0 ? "2px solid " + c2.bd : "none" }}>{l}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {MLABELS.map((m, mi) => {
              ...
              return (
                <tr key={mi} style={{ borderBottom: "0.5px solid var(--color-border-tertiary)" }}>
                  ...
                  {v1.map((v, i) => (
                    <td key={i} style={{ padding: "5px 7px", textAlign: "right", color: c1.tc, background: c1.cb, fontWeight: i === v1.length - 1 ? 500 : 400, borderLeft: i === 0 ? "2px solid " + c1.bd : "none" }}>{v ? fmt(v) : "-"}</td>
                  ))}
                  {v2.map((v, i) => (
                    <td key={i} style={{ padding: "5px 7px", textAlign: "right", color: c2.tc, background: c2.cb, fontWeight: i === v2.length - 1 ? 500 : 400, borderLeft: i === 0 ? "2px solid " + c2.bd : "none" }}>{v ? fmt(v) : "-"}</td>
                  ))}
            ...
            <tr style={{ borderTop: "1px solid var(--color-border-secondary)", fontWeight: 500, background: "var(--color-background-secondary)" }}>
              ...
              {c1.fs.map((f, i) => (
                <td key={i} style={{ padding: "5px 7px", textAlign: "right", color: c1.bd, borderLeft: i === 0 ? "2px solid " + c1.bd : "none" }}>{fmt(sumTo(data, year, f, upTo))}</td>
              ))}
              {c2.fs.map((f, i) => (
                <td key={i} style={{ padding: "5px 7px", textAlign: "right", color: c2.bd, borderLeft: i === 0 ? "2px solid " + c2.bd : "none" }}>{fmt(sumTo(data, year, f, upTo))}</td>
              ))}
...
// ---- Tab content components (no IIFEs) ----
        ...
        <div style={{ height: mob ? 180 : 250 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={bdRows} barSize={mob ? 7 : 10} barGap={2}>
              ...
              <Tooltip formatter={(v, n) => [Number(v).toLocaleString("ko-KR") + "원", n]}
                contentStyle={{ fontSize: 11, borderRadius: 6 }} />
...
function TabElec({ data, selYear, prevYear, mob, lmE, cliE, setCliE }) {
  ...
  const barRows = MLABELS.map((m, i) => {
    const d = data[mkey(selYear, i + 1)] || {};
    const pd = data[mkey(prevYear, i + 1)] || {};
    return { name: m, curr: ((d.e1_cost || 0) + (d.e2_cost || 0)) || null, prev: ((pd.e1_cost || 0) + (pd.e2_cost || 0)) || null };
  });
  const useRows = MLABELS.map((m, i) => {
    const d = data[mkey(selYear, i + 1)] || {};
    const pd = data[mkey(prevYear, i + 1)] || {};
    return { name: m, curr: ((d.e1_usage || 0) + (d.e2_usage || 0)) || null, prev: ((pd.e1_usage || 0) + (pd.e2_usage || 0)) || null, a: d.e1_usage || 0, b: d.e2_usage || 0 };
  });
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "repeat(4,minmax(0,1fr))", gap: 10, marginBottom: "1rem" }}>
...
function TabGas({ data, selYear, prevYear, mob, lmG, cliG, setCliG }) {
  ...
  const barRows = MLABELS.map((m, i) => {
    const d = data[mkey(selYear, i + 1)] || {};
    const pd = data[mkey(prevYear, i + 1)] || {};
    return { name: m, curr: ((d.gas1_cost || 0) + (d.gas2_cost || 0)) || null, prev: ((pd.gas1_cost || 0) + (pd.gas2_cost || 0)) || null };
  });
  const useRows = MLABELS.map((m, i) => {
    const d = data[mkey(selYear, i + 1)] || {};
    const pd = data[mkey(prevYear, i + 1)] || {};
    return { name: m, curr: ((d.gas1_usage || 0) + (d.gas2_usage || 0)) || null, prev: ((pd.gas1_usage || 0) + (pd.gas2_usage || 0)) || null, a: d.gas1_usage || 0, b: d.gas2_usage || 0 };
  });
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr 1fr" : "repeat(4,minmax(0,1fr))", gap: 10, marginBottom: "1rem" }}>
...
function TabCompare({ data, allYears, stored, mob }) {
  const rows4 = [
                ...
                {allYears.map((y) => {
                  ...
                  return (
                    <th key={y} style={{ padding: "6px 8px", textAlign: "right", color: "var(--color-text-secondary)", fontWeight: 500, whiteSpace: "nowrap" }}>
                      {y}년{lm2 > 0 && lm2 < 12 && <span style={{ fontSize: 9, marginLeft: 2, color: "#185FA5" }}>({lm2}월)</span>}
                    </th>
                  );
                })}
              </tr>
                  ...
                  {allYears.map((y, yi) => {
                    ...
                    return (
                      <td key={y} style={{ padding: "6px 8px", textAlign: "right", fontWeight: row.bold ? 500 : 400 }}>
                        {val > 0 ? fmt(val) : "-"}
                        {c != null && val > 0 && <span style={{ fontSize: 10, marginLeft: 3, color: c > 0 ? "#A32D2D" : "#3B6D11" }}>({fmtP(c)})</span>}
                      </td>
                    );
                  })}
                </tr>
                    ...
                    {allYears.map((y) => {
                      ...
                      return (
                        <th key={y} style={{ padding: "5px 7px", textAlign: "right", color: sec.color, fontWeight: 500, whiteSpace: "nowrap" }}>
                          {y}년{lm2 > 0 && lm2 < 12 && <span style={{ fontSize: 9, color: "#185FA5" }}>({lm2}월)</span>}
                        </th>
                      );
                    })}
                  </tr>
                      ...
                      {allYears.map((y, yi) => {
                        ...
                        return (
                          <td key={y} style={{ padding: "5px 7px", textAlign: "right" }}>
                            <div style={{ color: val ? "var(--color-text-primary)" : "var(--color-text-tertiary)", fontWeight: val ? 500 : 400 }}>{val ? fmt(val) : "-"}</div>
...
function TabEntry({ stored, setStored, mob }) {
  ...
  const saveFields = useCallback((fields) => {
    ...
    const ff = fields[0];
    setSaved((s) => Object.assign({}, s, { [ff]: true }));
    setTimeout(() => setSaved((s) => Object.assign({}, s, { [ff]: false })), 2000);
  }, [stored, form, eYear, eMonth, setStored]);
  ...
  return (
    <div>
      <div style={{ background: "#E6F1FB", border: "0.5px solid #B5D4F4", borderRadius: 8, padding: "10px 14px", marginBottom: "1rem" }}>
        ...
        <span style={{ fontSize: 12, color: "#185FA5", marginLeft: 10 }}>
          {"->"} 아래 데이터는 <strong>{uy}년 {um}월 사용량</strong>으로 반영됩니다
        </span>
      </div>
            ...
            return (
              <div key={acc.id} style={{ background: "var(--color-background-primary)", border: "0.5px solid " + acc.bd + "40", borderRadius: 12, padding: "1rem" }}>
                ...
                <div style={{ marginTop: "0.75rem", display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 8 }}>
                  ...
                  <button onClick={() => saveFields([acc.id + "_usage", acc.id + "_cost"])}
          ...
          {gAccs.map((acc) => {
            ...
            return (
              <div key={acc.id} style={{ background: "var(--color-background-primary)", border: "0.5px solid " + acc.bd + "40", borderRadius: 12, padding: "1rem" }}>
                ...
                <div style={{ marginTop: "0.75rem", display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 8 }}>
                  {saved[acc.id + "_usage"] && <span style={{ fontSize: 11, color: acc.color, fontWeight: 500 }}>저장됨!</span>}
                  <button onClick={() => saveFields([acc.id + "_usage", acc.id + "_heat", acc.id + "_cost"])}
                    style={{ padding: "6px 16px", fontSize: 12, fontWeight: 500, borderRadius: 6, border: "0.5px solid " + acc.bd, background: "transparent", color: acc.color, cursor: "pointer" }}>
                    {acc.label} 저장
                  </button>
                </div>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <button onClick={saveAll} style={{ padding: "10px 32px", fontSize: 13, fontWeight: 500, borderRadius: 8, border: "none", background: "#534AB7", color: "#fff", cursor: "pointer" }}>
          {saved.all ? "저장됨!" : "전체 저장"}
        </button>
        {saved.all && <span style={{ fontSize: 12, color: "#534AB7", fontWeight: 500 }}>전체 저장 완료!</span>}
...
export default function App() {
  ...
  if (loading) {
    return (
      <div style={{ padding: "2rem", color: "var(--color-text-secondary)" }}>불러오는 중...</div>
    );
  }
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", maxWidth: 980, margin: "0 auto", padding: mob ? "0.75rem 0.75rem 3rem" : "1rem 1rem 3rem" }}>
      ...
      {tab === "compare" && <TabCompare data={data} allYears={allYears} stored={stored} mob={mob} />}
